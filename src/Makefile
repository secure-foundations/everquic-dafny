...
DOTHs:=$(wildcard handwritten-dot-h-files/*.h)
...
clean-compile:
	...
	@rm -f $(DOTOs) $(CPPs) $(patsubst %.cpp,%.h,$(CPPs))
...
obj/%.cpp: %.dfy $(DAFNY_DEPS) | obj
	...
	| grep -v 'Additional code written to' \
	| sed '/^[[:space:]]*$$/d' \
	...
	@mv $(notdir $@) $@
	@clang-format -i $@
	@mv $(notdir $(patsubst %.cpp,%.h,$@)) $(patsubst %.cpp,%.h,$@)
	@clang-format -i $(patsubst %.cpp,%.h,$@)
...
.depend: $(DFYs) Makefile
	...
	@echo 'else' >> $@
	@for i in $(DFYs); do awk '/^include \".*dfy\"$$/{gsub("\"","",$$2); print "obj/'$$i'.ver: " $$2 "\n"}' $$i; done >> $@
	@for i in $(DFYs); do awk '/^include \".*dfy\"$$/{gsub("\"","",$$2); print "obj/'$$i'.versyn: " $$2 "\n"}' $$i; done >> $@
	@for i in $(DFYs); do awk '/^include \".*dfy\"$$/{gsub("\"","",$$2); print "obj/'$$i'x: " $$2 "\n"}' $$i; done | sed 's/\.dfyx/\.cpp/g' >> $@
	@for i in $(DFYs); do awk '/^include \".*dfy\"$$/{gsub("\"","",$$2); print "obj/'$$i'x: " $$2 "\n"}' $$i; done | sed 's/\.dfyx/\.o/g' >> $@
	@for i in $(DFYs); do awk '/^include \".*dfy\"$$/{gsub("\"","",$$2); print "obj/'$$i'x: " $$2 "\n"}' $$i; done | sed 's/\.dfyx/\.cs/g' >> $@
	@echo 'endif' >> $@
